...
export class MetadataManager {
    ...
    initializeFromProjectSettings(projectStructure) {
        ...
        const statusContainer = (statusSettings?.StatusItems || statusSettings?.StatusList);
        if (statusContainer) {
     ...
     */
    updateDocumentMetadata(item, metadata) {
        if (!item) {
    ...
    getDocumentMetadata(item) {
        ...
        if (item.MetaData) {
            ...
            if (metaData.CustomMetaData?.MetaDataItem) {
                metadata.customMetadata = this.extractCustomMetadata(metaData.CustomMetaData.MetaDataItem);
            }
        }
        return metadata;
    }
     ...
     */
    async batchUpdateMetadata(items, updates) {
        const results = [];
     ...
     */
    updateProjectMetadata(projectStructure, metadata) {
        ...
        if (metadata.customFields) {
            ...
            Object.assign(settingsAny.CustomFields, metadata.customFields);
        }
     ...
     */
    getProjectMetadata(projectStructure) {
        if (!projectStructure?.ScrivenerProject) {
            return {};
        }
        const project = projectStructure.ScrivenerProject;
        ...
        if (project.ProjectSettings) {
            ...
            const settingsAny = settings;
            if (settingsAny.CustomFields) {
                metadata.customFields = { ...settingsAny.CustomFields };
            }
        }
        if (project.ProjectTargets) {
            const targets = project.ProjectTargets;
     ...
     */
    searchMetadata(items, query, fields = [
        'title',
        'synopsis',
    ]) {
        const results = [];
        ...
        const searchItem = (item) => {
            ...
            if (item.MetaData) {
                ...
                if (fields.includes('keywords') && item.MetaData.Keywords) {
                    const keywords = typeof item.MetaData.Keywords === 'string'
                ...
                if (fields.includes('custom') && item.MetaData.CustomMetaData?.MetaDataItem) {
                    const customData = this.extractCustomMetadata(item.MetaData.CustomMetaData.MetaDataItem);
        ...
        const processItem = (item) => {
            ...
            if (item.MetaData) {
                ...
                if (item.MetaData.CustomMetaData?.MetaDataItem) {
                    ...
                    const custom = this.extractCustomMetadata(item.MetaData.CustomMetaData.MetaDataItem);
    ...
    updateCustomMetadata(metaData, customFields) {
        if (!metaData) {
            return;
        }
        if (!metaData.CustomMetaData) {
        ...
        for (const [key, value] of Object.entries(customFields)) {
            const existing = customMetaItems.find((item) => item?.ID === key);
            if (existing) {
    ...
    validateMetadataInput(metadata) {
        if (metadata.title && metadata.title.length > this.maxStringLength) {
            throw new AppError(`Title too long (max ${this.maxStringLength} characters)`, ErrorCode.INVALID_INPUT);
        }
        if (metadata.synopsis && metadata.synopsis.length > this.maxStringLength) {
            throw new AppError(`Synopsis too long (max ${this.maxStringLength} characters)`, ErrorCode.INVALID_INPUT);
        }
        if (metadata.notes && metadata.notes.length > this.maxStringLength) {
            throw new AppError(`Notes too long (max ${this.maxStringLength} characters)`, ErrorCode.INVALID_INPUT);
        }
        ...
        if (metadata.customMetadata) {
            ...
            if (customCount > 50) {
                throw new AppError('Too many custom metadata fields (max 50)', ErrorCode.INVALID_INPUT);
            }
            for (const [key, value] of Object.entries(metadata.customMetadata)) {
                if (key.length > 100) {
                    throw new AppError(`Custom field key too long: ${StringUtils.truncate(key, 50)}`, ErrorCode.INVALID_INPUT);
                }
                if (value && value.length > this.maxStringLength) {
                    throw new AppError(`Custom field value too long for key: ${key}`, ErrorCode.INVALID_INPUT);
                }
                ...
                if (key.toLowerCase().includes('url') || key.toLowerCase().includes('link')) {
                    if (value && !ValidationUtils.validateUrl(value)) {
                        throw new AppError(`Invalid URL in custom field '${key}': ${value}`, ErrorCode.INVALID_INPUT);
                    }
                }
            }
        }
     ...
     */
    validateProjectMetadataInput(metadata) {
        if (metadata.title && metadata.title.length > this.maxStringLength) {
            throw new AppError(`Project title too long (max ${this.maxStringLength} characters)`, ErrorCode.INVALID_INPUT);
        }
        if (metadata.author && metadata.author.length > 500) {
            throw new AppError('Author name too long (max 500 characters)', ErrorCode.INVALID_INPUT);
        }
        if (metadata.projectTargets) {
            const { draft, session } = metadata.projectTargets;
            if (draft !== undefined && (draft < 0 || draft > 10000000)) {
                throw new AppError('Draft target must be between 0 and 10,000,000', ErrorCode.INVALID_INPUT);
            }
            if (session !== undefined && (session < 0 || session > 100000)) {
                throw new AppError('Session target must be between 0 and 100,000', ErrorCode.INVALID_INPUT);
            }
        }
    }
    /**
     ...
     */
    async batchProcessMetadata(items, processor, options = {}) {
        const { concurrency = 5, continueOnError = true, timeoutMs = 30000, // 30 seconds timeout per item
        retryOptions = { maxAttempts: 2, initialDelay: 500, maxDelay: 2000 }, } = options;
        const results = [];
        ...
        for (let i = 0; i < items.length; i += concurrency) {
            ...
            const batchPromises = batch.map(async (item) => {
                try {
                    ...
                    const result = await AsyncUtils.withTimeout(AsyncUtils.retryWithBackoff(async () => {
                        return await processor(item);
                    }, retryOptions), timeoutMs, `Processing item ${item.UUID || item.ID} timed out after ${timeoutMs}ms`);
                    return { item, result };
                }
                catch (error) {
     ...
     */
    async validateMultipleItems(items, requiredFields = [], options = {}) {
        const { concurrency = 10 } = options;
        return await this.batchProcessMetadata(items, (item) => {
     ...
     */
    async batchValidateAndRepair(items, repairOptions = {}, processingOptions = {}) {
        const { concurrency = 5, timeoutMs = 10000 } = processingOptions;
        const { addMissingTitles = true, setDefaultStatus = true, defaultStatus = 'Draft', } = repairOptions;
        return await this.batchProcessMetadata(items, async (item) => {
            const changes = [];
            let repaired = false;
     ...
     */
    async searchMetadataAsync(items, query, fields = [
        'title',
        'synopsis',
    ], options = {}) {
        ...
        try {
            const results = await AsyncUtils.withTimeout(Promise.resolve(this.searchMetadata(items, query, fields)), timeout, `Metadata search timed out after ${timeout}ms`);
