import { validateInput, isValidUUID, truncate, createApiResponse, measureExecution, createError, ErrorCode, handleError, } from '../utils/common.js';
...
import { documentContentSchema, documentIdSchema, documentMoveSchema, documentTitleSchema, } from './validation-schemas.js';
export const getDocumentInfoHandler = {
                ...
                content: [
                    {
                        type: 'text',
                        text: result.result,
                        data: createApiResponse({ contentLength: result.result.length }, { executionTime: result.ms }),
                    },
                ],
            };
        }
...
export const writeDocumentHandler = {
    name: 'write_document',
                ...
                content: [
                    {
                        ...
                        data: createApiResponse({ documentId, contentLength: content.length }, { executionTime: result.ms }),
                    },
                ],
            };
        }
...
export const createDocumentHandler = {
    ...
    handler: async (args, context) => {
        try {
            ...
            const documentType = (getOptionalStringArg(args, 'documentType') || 'Text');
            // Validate parent ID if provided
            if (parentId && !isValidUUID(parentId)) {
                throw createError(ErrorCode.INVALID_INPUT, { parentId }, 'Invalid parent ID format');
            }
            const result = await measureExecution(() => project.createDocument(title, content, parentId, documentType));
                ...
                content: [
                    {
                        ...
                        data: createApiResponse({ documentId: result.result, title, documentType }, { executionTime: result.ms, contentLength: content.length }),
                    },
                ],
            };
        }
...
export const renameDocumentHandler = {
...
export const updateMetadataHandler = {
    ...
    handler: async (args, context) => {
        ...
        if (synopsis)
            updates.push(`synopsis set`);
        if (notes)
            updates.push(`notes set`);
        return {
            content: [
                {
                    type: 'text',
...
export const getWordCountHandler = {
    name: 'get_word_count',
    ...
    handler: async (args, context) => {
        ...
        if (documentId) {
            ...
            if (includeChildren) {
                ...
                const childDocs = allDocs.filter((doc) => doc.path && doc.path.includes(documentId) && doc.id !== documentId);
                ...
                for (let i = 0; i < childDocs.length; i += batchSize) {
                    ...
                    const batchCounts = await Promise.all(batch.map(async (doc) => {
...
export const readFormattedHandler = {
    ...
    handler: async (args, _context) => {
        try {
            ...
            const resultsText = filtered
                .map((result, index) => `${index + 1}. ${result.entry.id} (similarity: ${result.similarity.toFixed(3)})
` + `   ${result.explanation || 'No explanation available'}`)
            ...
            if (error.message.includes('HHM system not initialized')) {
                return {
                    content: [
                        {
                            type: 'text',
...
export const findAnalogiesHandler = {
    name: 'find_analogies',
    ...
    handler: async (args, _context) => {
        try {
            ...
            const analogiesText = results
                .map((result, index) => `${index + 1}. ${a}:${b} :: ${c}:${result.entry.id} (confidence: ${result.similarity.toFixed(3)})`)
                .join('\n');
            return {
                content: [
                    {
