...
import { documentIndexer } from './services/document-indexer.js';
...
import { createDocument as createDocumentUtil, } from './utils/document-operations.js';
...
export class ScrivenerProject {
    ...
    async loadProject() {
        ...
        this.contextSync = new ContextSyncService(this.projectPath, this.databaseService, this.contextAnalyzer, {
            autoSync: true,
    ...
    async writeDocument(documentId, content) {
    ...
    async moveDocument(documentId, newParentId, _position) {
    ...
    async compileDocuments(documentIds, separator = '\n\n---\n\n', outputFormat = 'text') {
        const documents = [];
    ...
    async exportProject(format, _outputPath, options) {
    ...
    async batchUpdateSynopsisAndNotes(updates) {
        const results = [];
    ...
    async analyzeChapterEnhanced(documentId) {
        ...
        return (await this.contextAnalyzer.analyzeChapter(documentWithCharCount, content, documentsWithCharCount));
    }
    ...
    async getDocumentInfo(documentId) {
        ...
        const searchInDocs = (docs, currentPath) => {
    ...
    async searchTrash(query, options) {
    ...
    async performInitialSync() {
        ...
        for (let i = 0; i < allDocs.length; i += BATCH_SIZE) {
            ...
            logger.debug(`Synced ${Math.min(i + BATCH_SIZE, allDocs.length)}/${allDocs.length} documents`);
        }
    }
    async syncDocumentToDatabase(doc) {
        try {
            let wordCount = 0;
    ...
    findBinderItem(structure, documentId) {
        const scrivenerProject = structure.ScrivenerProject;
        if (!scrivenerProject?.Binder)
            return null;
...
//# sourceMappingURL=scrivener-project.js.map